...
def BeatNet_beats(filename):
    """
    Use of BeatNet beat tracking, for better beat and downbeat tracking.
    https://github.com/mjhydri/BeatNet

    It returns a 2D array that also contains the beat order info for each
    time (i.e. [1, 1.23s], [2, 2.34s]) which we'll later need, hence the
    inconsistent returns.
    """
    estimator = BeatNet(1, mode='offline', inference_model='DBN', plot=[], thread=False)
    beats = estimator.process(filename)
...
def make_beat_sync_features(filename, 
    ...
    ------
    Exception
        beats_algo must be "madmom", "beatnet", or "librosa", otherwaise raises and error
    """
